    ...
    rotate(rot_vec) {
      if(part == "block") {
        translate([mounting_screw_block_width / -2, (mounting_screw_block_width / -2) - wall_centerline_height, block_trans_z]) { // translate to center on XY
          cube([mounting_screw_block_width, mounting_screw_block_width + wall_centerline_height, mounting_screw_bore_length + mounting_nut_height + bore_extra_length]);
        } // translate to center on XY
      } else {
        translate([0, -1 * wall_centerline_height, 0]) {
          // nut trap itself
          translate([0, 0, mounting_screw_bore_length + bore_extra_length - 0.01]) {
            cylinder(r = mounting_nut_key / 2 / cos(180 / 6) + 0.05, h=mounting_nut_height + 0.1, $fn=6);
          } // translate
          // the screw clearance bore
          cylinder(d=mounting_screw_bore_dia, h=mounting_screw_bore_length + bore_extra_length, $fn=360);
        } // translate
      } // if part
    } // rotate
  } // translate
...
module mounting_nut_block_with_hole(face, wall_centerline_height = 0, bore_extra_length = 0) {
  include <config.scad>;
  difference() {
    mounting_nut_catch(part="block", face=face, wall_centerline_height=wall_centerline_height, bore_extra_length=bore_extra_length);
    tr = face == "base" ? [0, 0, (-1 * (initial_layer_height + layer_height + 0.001))] : [0, 0, 0];
    translate(tr) {
      mounting_nut_catch(part="hole", face=face, wall_centerline_height=wall_centerline_height, bore_extra_length=bore_extra_length);
    }
  }
}

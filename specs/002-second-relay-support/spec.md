...
- **FR-006**: Machines without a `second_relay` block MUST behave identically to today, including identical LCD content, structured log output, and Slack message content for all events.
...
- **SC-003**: After this feature ships, an existing `machines.json` file that contains no `second_relay` blocks produces no observable behavior change versus the pre-feature server in regression testing — no new log lines, no new Slack messages, no new metric series for those machines.

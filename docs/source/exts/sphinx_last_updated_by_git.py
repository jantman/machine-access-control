...
# flake8: noqa
# vendored-in to get unreleased commit:
# https://github.com/mgeier/sphinx-last-updated-by-git/commit/1a1e4d564ea5e73c8c0c880e36d1490a3f9c08fe
    ...
    * If the root commit is reached (i.e. there is at least one of the
    ...
    existing_files = (
        subprocess.check_output(
            ["git", "ls-tree", "--name-only", "-z", "HEAD", "--", *requested_files],
            cwd=git_dir,
    ...
    process = subprocess.Popen(
        [
            "git",
            "log",
            "--pretty=format:%n%at%x00%H%x00%P",
            "--author-date-order",
            "--relative",
            "--name-only",
            "--no-show-signature",
    ...
    with process:
        parse_log(process.stdout, requested_files, git_dir, exclude_commits, file_dates)
...
def parse_log(stream, requested_files, git_dir, exclude_commits, file_dates):
    requested_files = {f.encode("utf-8") for f in requested_files}
...
def _env_updated(app, env):
    ...
    for git_dir in srcdir_iter:
        try:
            ...
            logger.warning(
                '"git" command not found, ' 'no "last updated" dates will be shown',
                type="git",
...
def _html_page_context(app, pagename, templatename, context, doctree):
    ...
    context["last_updated"] = format_date(
        lufmt or _("%b %d, %Y"), date=date, language=app.config.language
    )
...
def setup(app):
    ...
    app.add_config_value("git_untracked_check_dependencies", True, rebuild="env")
    app.add_config_value("git_untracked_show_sourcelink", False, rebuild="env")
    app.add_config_value("git_last_updated_timezone", None, rebuild="env")
    app.add_config_value("git_last_updated_metatags", True, rebuild="html")
    app.add_config_value("git_exclude_patterns", [], rebuild="env")
    app.add_config_value("git_exclude_commits", [], rebuild="env")
    return {
        "version": __version__,
